...
public class BluetoothActivity extends Activity {
    ...
    private final ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder service) {
            mGattService = ((GattService.LocalBinder) service).getService();
            if (!mGattService.initialize()) {
                Log.e(TAG, "Unable to initialize Bluetooth");
                finish();
            }
            // Automatically connects to the device upon successful start-up initialization.
            mGattService.connect(mDeviceAddress);
        }
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (GattService.ACTION_GATT_CONNECTED.equals(action)) {
                ...
                invalidateOptionsMenu();
                clearUI();
            } else if (GattService.ACTION_GATT_SERVICES_DISCOVERED.equals(action)) {
                // Show all the supported services and characteristics on the user interface.
                //displayGattServices(mBluetoothLeService.getSupportedGattServices());
            } else if (GattService.ACTION_DATA_AVAILABLE.equals(action)) {
                displayData(intent.getStringExtra(GattService.EXTRA_DATA));
            }
        }
    };
                    ...
                    if (mGattCharacteristics != null) {
                        ...
                        if ((charaProp | BluetoothGattCharacteristic.PROPERTY_READ) > 0) {
                            ...
                            if (mNotifyCharacteristic != null) {
                                mGattService.setCharacteristicNotification(
                                        mNotifyCharacteristic, false);
                                mNotifyCharacteristic = null;
                            }
                            mGattService.readCharacteristic(characteristic);
                        }
    ...
    */
    ...
    protected void onResume() {
        ...
        if (mGattService != null) {
            final boolean result = mGattService.connect(mDeviceAddress);
    ...
    public void onClickWrite(View v){
        if(mGattService != null) {
            mGattService.writeCustomCharacteristic(0xAA);
        }
    }

    public void onClickRead(View v){
        if(mGattService != null) {
            mGattService.readCustomCharacteristic();
        }
    }
